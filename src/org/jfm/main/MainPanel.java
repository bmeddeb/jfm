...
//  TASK H2 *************************************************************************
	...
	String[] rpm = RolesSingleton.getRoleMapping().getPrivilegesForRole(userRole); 
		...
		Map<String, JButton> actionButtonMapping = new HashMap<>();

		actionButtonMapping.put("View", f3Button);
		actionButtonMapping.put("Edit", f4Button);
		actionButtonMapping.put("Copy", f5Button);
		actionButtonMapping.put("Move", f6Button);
		actionButtonMapping.put("Mkdir", f7Button);
		RolesSingleton.getRoleMapping().getPrivilegesForRole(userRole);
		// Add buttons based on privileges
		for (String privilege : rpm) {
			if (actionButtonMapping.containsKey(privilege)) {
				buttons.addElement(actionButtonMapping.get(privilege));
			}
		}
...
// TASK H3 *******************************************************************************
		...
		changePasswordButton.addActionListener(changePasswordAction);
